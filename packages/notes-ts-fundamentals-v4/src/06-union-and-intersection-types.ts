...
// let evenOrLowNumber: OneThroughFiveOrEvens = 7; //! 7 is not in either set(/type)
...
const success = ["success", { name: "Mike North", email: "mike@example.com" }] as const //? the `as const` is a TypeScript feature that makes the array a tuple 
...
const [type, obj] = maybeGetUserInfo()

//✔️ TypeScript only knows about the properties common to both types (which is
//✔️ `name` in this case). It doesn't know whether obj is an error or user info
obj.name

if (type === "error") {
    obj.message //✔️ TypeScript knows this is an Error
} else {
    obj.email //✔️ TypeScript knows this is the user info
}
